@isset ($page)
    ...
    <title>
        @yield ('seo_title')
        {!! $page->seo_title ?: $page->title !!} {{ $seo->title_separator }} {!! $seo->site_name ?: config('app.name') !!}
    </title>
    {{-- Page description --}}
    ...
    @if ($seo->hreflang_auto)
        @if (! $page->seo_noindex && $page->seo_canonical_type->value() === 'entry' && $current_full_url === $permalink->value())
            <s:locales all="false">
                <link
                    rel="alternate"
                    hreflang="{{ str_replace('_','-', $locale['full']) }}"
                    href="{{ $locale['permalink'] }}"
                />
            </s:locales>
        @endif
    @endif
    ...
    @unless ($page->seo_noindex)
        ...
        @elseif ($page->seo_canonical_type->value() === 'domain')
            <link
                rel="canonical"
                href="{{ trim(config('app.url'), '/') . $page->seo_canonical_domain->url }}"
            />
    ...
    @isset ($page->schema_jsonld)
        <script type="application/ld+json" id="schema-page">
            {!! $page->schema_jsonld !!}
        </script>
    @endisset
    ...
    @if ($seo->breadcrumbs && !empty($segment_1))
        ...
        <script type="application/ld+json" id="schema-breadcrumbs">
            @json($breadcrumbsSchema, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE)
        </script>
    @endif
    {{-- Open Graph --}}
    ...
    @if ($page->og_description || $page->seo_description)
        <meta
            ...
            content="{{ $page->og_description ?: $page->seo_description }}"
        />
    @elseif ($seo->collection_defaults)
    ...
    @if ($twitter_image = $page->twitter_image ?: $seo->twitter_image)
        ...
        <meta
            name="twitter:site"
            content="{{ Statamic::modify($seo->twitter_handle)->ensure_left('@') }}"
        />
        <meta
            name="twitter:title"
            content="{{ $page->og_title ?: $page->seo_title ?: $page->title }}"
        />
        @if ($page->og_description || $page->seo_description)
            <meta
...
@if (
    ...
    @elseif ($seo->tracker_type == 'gtag')
        ...
        ></script>
        <script>
            window.dataLayer = window.dataLayer || [];function gtag(){dataLayer.push(arguments);}gtag('js', new Date());gtag('set', new Date());gtag('config', '{{ $seo->google_analytics }}' {{ $seo->anonymize_ip ? ", {'anonymize_ip': true}" : '' }});
        </script>
    @endif
    ...
    @if ($seo->use_cloudflare_web_analytics)
        <script
            ...
            data-cf-beacon='{"token": "{{ $seo->cloudflare_web_analytics }}"}'
        ></script>
    @endif
@endif
