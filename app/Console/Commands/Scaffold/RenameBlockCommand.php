...
final class RenameBlockCommand extends Command
{
    ...
    public function handle(
        ...
        $currentHandle = $this->resolveBlock($sets);

        $currentName = $sets[$currentHandle] ?? null;
        if ($currentName === null) {
            error("The '{$currentHandle}' block was not found in the '{$groups[$currentGroup]}' group.");
        ...
        try {
            ...
            $updatedEntries = $this->renameBlock($moveScaffoldFiles, $renameSetUsages, $currentGroup, $targetGroup, $currentHandle, $currentName, $new);
